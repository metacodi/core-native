...
import { NativeConfig, PluginType } from '../native-config';
...
export class StoragePlugin {
  protected debug = true && NativeConfig.debugEnabled && NativeConfig.debugPlugins.includes(this.constructor.name as PluginType);
  ...
  async length(moduleName?: string): Promise<any> {
